...
| processName | PatternProcess | No | PatternProcess.NEW_PATTERN | `PatternProcess` contains two processes: `NEW_PATTERN` and `CONFIRM_PATTERN`. For the Change Pattern process, set to `CONFIRM_PATTERN`. |
...
| enableDotsJoinVibration | boolean | No | false | If set to `true`, the mobile will vibrate whenever pattern dots are connected (ensure vibration permissions are granted). [Refer React Native Vibration](https://reactnative.dev/docs/vibration) |
...
| enablePatternNotSameCondition | boolean | No | true | If `true`, previous pattern cannot be used as new pattern during Change Pattern flow. If `false`, previous pattern can be reused. |

| hintMessages | HintMessages | No | {} | Object containing context-specific hint messages for each pattern flow stage and event (see HintMessages reference below) |
